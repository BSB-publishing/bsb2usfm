...
class Processor:
    ...
    def makebook(self, bk):
        ...
        mt2, mt1 = booktitles.get(bk, ("", books[1]))
        mt2_line = f'  <para style="mt2">{mt2}</para>\n' if mt2 else ''
...
{4}  <para style="mt1">{5}</para>
</usx>""".format(bk, *books, mt2_line, mt1, self.identifier)
    ...
    def processline(self, row):
        ...
        if f['End text']:
            end_text = f['End text'] if self.brackets else debracket(f['End text'])
            self.addend(end_text)
...
parser.add_argument("-t","--titles",help="Book titles JSON file: {\"CODE\": [\"mt2\", \"mt1\"], ...} for \\mt1/\\mt2 overrides")
...
if args.titles:
    with open(args.titles, encoding="utf-8") as inf:
        for code, pair in json.load(inf).items():
            mt2, mt1 = pair
            booktitles[code] = (mt2.strip(), mt1.strip())
