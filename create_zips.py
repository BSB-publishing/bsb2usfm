...
- Places the zip file in the same directory as the source files
"""
...
def get_common_extension(filenames: List[str]) -> str:
    ...
    extensions = [os.path.splitext(f)[1].lstrip('.') for f in filenames if os.path.splitext(f)[1]]
...
def create_zip_for_directory(directory: Path, dry_run: bool = False, verbose: bool = False):
    """
    Create or update a zip file for the given directory.
    The zip file will be created in the same directory as the source files.
    ...
    files = [f for f in directory.iterdir() if f.is_file() and not f.name.endswith('.zip')]
    ...
    # Create the zip file
    if dry_run:
        print(f"  [DRY RUN] Would create {zip_path.relative_to(Path.cwd())} with {len(files)} files")
        return
...
def process_branch(branch_path: Path, dry_run: bool = False, verbose: bool = False):
    """
    ...
    root_files = [f for f in branch_path.iterdir() if f.is_file() and not f.name.endswith('.zip')]
    if root_files:
        if verbose:
            print(f"  Root directory ({len(root_files)} files):")
        create_zip_for_directory(branch_path, dry_run=dry_run, verbose=verbose)
    ...
    for subdir in sorted(subdirs):
        ...
        create_zip_for_directory(subdir, dry_run=dry_run, verbose=verbose)


def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(
        description="Create/update zip files for results and results_usj directories",
...
Examples:
  ...
  %(prog)s -n -v              # Dry run with verbose output
    ...
    for branch in branches:
        process_branch(branch, dry_run=args.dry_run, verbose=args.verbose)
